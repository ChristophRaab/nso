function [Xs, Xt, Ys, Yt, acc_oracle] = hl_ldada(Xs,Xt,Ys,Yt, options)
...
P.nclasses = size(unique(Ys),1);
options.nclasses = size(unique(Ys),1); 
% init W
W = eye(size(Xs, 1));
Wp = ones(size(Xs, 1), options.nclasses);
